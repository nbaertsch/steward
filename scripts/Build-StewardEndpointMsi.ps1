...
if ($TestBuild) {
    ...
    if ($env:GITHUB_ACTIONS -ne 'true' -or
        ...
        throw 'Production endpoint MSI builds require complete GitHub Actions provenance.'
    }
}

$projects = @(
    'src\Steward.RdpDvc.Server.Windows\Steward.RdpDvc.Server.Windows.csproj',
    'src\Steward.HandleKeeper\Steward.HandleKeeper.csproj',
    'src\Steward.Endpoint.Provisioner\Steward.Endpoint.Provisioner.csproj'
)
foreach ($project in $projects) {
    dotnet publish (Join-Path $root $project) -c Release -r win-x64 `
        --self-contained true -o $payload `
...
foreach ($file in Get-ChildItem -LiteralPath $payload -File) {
    if ($excluded | Where-Object { $file.Name.StartsWith($_, 'OrdinalIgnoreCase') }) {
