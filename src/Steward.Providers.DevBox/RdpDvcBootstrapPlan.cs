...
public static class DevBoxRdpDvcBootstrapPlan
{
            ...
            RdpDvcBootstrapBundle bundle)
    {
        ...
        var startupSource = Convert.ToBase64String(
            Encoding.UTF8.GetBytes(
                ...
                "$reconnectLedgerPath=Join-Path $runDirectory 'reconnect-ledger.v2.db'\r\n" +
                "$readinessPath=Join-Path $runDirectory 'endpoint-health.v2.json'\r\n" +
                ...
                "$serverArgs=@('exec','--runtimeconfig',$runtimeConfig,'--depsfile',$depsFile,$server,'--session-id','" + sessionId + "','--host-id','" + hostId + "','--incarnation-id','" + incarnationId + "','--auth-key-file',$keyPath,'--reconnect-ledger-file',$reconnectLedgerPath,'--readiness-receipt-file',$readinessPath,'--node-host-config',$nodeConfig,'--portable-state-root',$portable,'--credential-vault-root',$credentials,'--node-signing-key-file',$nodeSigningPath,'--node-identity','" + request.NodeTransportIdentity + "','--control-signing-key-file',$controlSigningPath,'--control-identity','" + request.ControlTransportIdentity + "')\r\n" +
        ...
        DevBoxRdpDvcBootstrapRequest request)
    {
        ...
        var command =
            "$ErrorActionPreference='Stop';" +
            $"$readinessPattern='C:\\Users\\*\\AppData\\Local\\StewardNode\\rdp-dvc\\runs\\{operationId}\\endpoint-health.v2.json';" +
        ...
        string authenticationKey)
    {
        ...
        $"$encoded=((0..{chunkCount - 1}|ForEach-Object{{Get-Content -LiteralPath (Join-Path $chunks ($_.ToString('D4')+'.txt')) -Raw}}) -join '');" +
               ...
               $"$runDirectory=Join-Path $userRoot 'runs\\{operationId}';[void](New-Item -ItemType Directory -Path $runDirectory -Force);$reconnectLedgerPath=Join-Path $runDirectory 'reconnect-ledger.v2.db';" +
               ...
               "$readinessPath=Join-Path $runDirectory 'endpoint-health.v2.json';" +
               $"$arguments=('\"'+$server+'\" --session-id {sessionId} --host-id {hostId} --incarnation-id {incarnationId} --auth-key-file \"'+$keyPath+'\" --reconnect-ledger-file \"'+$reconnectLedgerPath+'\" --readiness-receipt-file \"'+$readinessPath+'\" --node-host-config \"'+$nodeHostConfigPath+'\" --portable-state-root \"'+$portableStateRoot+'\" --credential-vault-root \"'+$credentialVaultRoot+'\"');" +
               ...
               "$launchArgs=@('exec','--runtimeconfig',$runtimeConfig,'--depsfile',$depsFile,$server,'--session-id','" + sessionId + "','--host-id','" + hostId + "','--incarnation-id','" + incarnationId + "','--auth-key-file',$keyPath,'--reconnect-ledger-file',$reconnectLedgerPath,'--readiness-receipt-file',$readinessPath,'--node-host-config',$nodeHostConfigPath,'--portable-state-root',$portableStateRoot,'--credential-vault-root',$credentialVaultRoot" + secureArguments + ");" +
               "$serverProcessId=[StewardSessionLauncher]::Start($userSession,$dotnet,$target,$launchArgs);if($serverProcessId -lt 0){throw ('RDP DVC endpoint failed to start '+(-$serverProcessId))};Write-Output ('STEWARD_RDP_DVC_SERVER_PID:'+$serverProcessId);" +
               "$readinessDeadline=[DateTime]::UtcNow.AddMinutes(14);$remote=$null;do{Start-Sleep -Seconds 2;if(Test-Path -LiteralPath $readinessPath){try{$remote=Get-Content -LiteralPath $readinessPath -Raw|ConvertFrom-Json}catch{$remote=$null}};$running=$null-ne(Get-Process -Id $serverProcessId -ErrorAction SilentlyContinue);if(!$running){$failurePath=$readinessPath+'.failure';$failure=if(Test-Path -LiteralPath $failurePath){Get-Content -LiteralPath $failurePath -Raw}else{'unknown'};throw ('RDP DVC endpoint failed '+$failure)}}while(($null-eq$remote -or $remote.state -ne 'authenticated') -and [DateTime]::UtcNow -lt $readinessDeadline);" +
               "if($null-eq$remote -or $remote.state -ne 'authenticated'){throw 'RDP DVC endpoint did not authenticate reconnect generation'};$dvcReady=$true;" +
